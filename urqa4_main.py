...
def objective_exposureV3(exposure_mat, stations, diversity_d, diversity_median, workers):
	...
	f_sdiv = np.sum(scores_div) / (1 + np.std(scores_div)**2)
	f_sexp = np.sum(scores_exp) * (1 + np.std(scores_exp)**2)
...
#worker versatility
print(r"GA_tests\URQA4\Matriz versatilidade_A4B.xlsx")
